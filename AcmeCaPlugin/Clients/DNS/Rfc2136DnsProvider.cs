...
namespace Keyfactor.Extensions.CAPlugin.Acme
{
    ...
    public class Rfc2136DnsProvider : IDnsProvider
    {
        ...
        /// <summary>
        /// Encodes a domain name in DNS wire format with lowercase (canonical form for TSIG).
        /// </summary>
        private byte[] EncodeDomainNameLowercase(string name)
        {
            var result = new List<byte>();
            var labels = name.TrimEnd('.').ToLowerInvariant().Split('.');
